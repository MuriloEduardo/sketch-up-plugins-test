...
Tests in `tests/sketchup/` use the [TestUp](https://github.com/SketchUp/testup-2) framework (`TestUp::TestCase`). Tests run inside SketchUp, not standalone Ruby.
...
## Unit tests outside SketchUp

Pure-Ruby logic (no `Sketchup::`, `UI::`, `Geom::` or `VRay::`) is tested with plain
Minitest in `tests/unit/**/*_test.rb` and runs in Docker with `make test`. Load
source files with `require_source 'me_vray_toolkit/...'` from `tests/unit/test_helper.rb`.
Prefer moving logic into pure modules so it can be tested here.
